...
std::tuple<py::array, py::array> thsadf(py::array_t<float> data, int maxw, int minw, int p, bool usedrift, float gpumem_gb, bool verbose){
    ...
    outsadf.resize(indata.size() - maxw);
    outadfmaxidx.resize(indata.size() - maxw);
    int ressize = sadf(indata.data(), outsadf.data(), outadfmaxidx.data(), indata.size(), maxw, minw, p, usedrift, gpumem_gb, verbose);

    return std::make_tuple(py::cast(outsadf), py::cast(outadfmaxidx));
}
